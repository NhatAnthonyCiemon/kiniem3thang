...
const authController = {
    refreshToken: async (req, res) => {
        ...
        try {
            const userId = await User.getUserIdByRefreshToken(refreshToken);
            if (!userId) {
                return res
                    .status(401)
    ...
    getMe: async (req, res) => {
        try {
            ...
            if (!user) {
                return res
                    .status(404)
                    .json(createErrorResponse(404, "User not found"));
            }
            res.status(200).json(
                createResponse(200, "success", {
                    id: user.id,
    ...
    loginLocal: async (req, res) => {
        try {
            ...
            if (user.is_verify === false) {
                return res
                    .status(400)
                    .json(createErrorResponse(400, "Email not verified"));
            }
            const payload = {
                id: user.id,
                email: user.email,
